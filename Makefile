...
key-pair.create: ##=> Checks if the key pair already exists and creates it if it does not
	...
	@EC2_KEY_PAIR_NAME="$$(jq -r '.layers.graph_database.env.ssh_key_name' ${ROOT_DIR}/config.json)" && \
...
neo4j.wait:
	@endpoint=$$(aws ssm get-parameters \
		--names "/${ORGANIZATION}/${APP_NAME}/Neo4jEndpoint" | jq -r '.Parameters[0].Value') && \
	timeout=360 && \
	counter=0 && \
	echo "Waiting for response from Neo4j at $$endpoint..." && \
...
neo4j.get.endpoint:
	@endpoint=$$(aws ssm get-parameters --names "/${ORGANIZATION}/${APP_NAME}/Neo4jEndpoint" | jq -r '.Parameters[0].Value') && \
	echo "$$endpoint"
